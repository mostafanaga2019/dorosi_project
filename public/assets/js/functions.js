;(function($) {
    ...
    var MERCADO_JS = {
    	init: function(){
    		...
    		this.mercado_remove_service_in_cart();
    		this.mercado_service_slider();
    	...
    	mercado_service_slider: function(){
	    ...
	    mercado_chosen: function(){
			if($('.wrap-search-form .wrap-list-cate').length > 0){
                ...
                $('.wrap-search-form .wrap-list-cate .list-cate').on('click', 'li', function (event) {
                    ...
                    _this.parent().siblings('input[name="center_division"]').val(_value);
					_this.parent().siblings('input[name="center_division_id"]').val(_this.data("id"));					
		...
		mercado_service_chosen: function(){
			if($('.wrap-reserve-form .wrap-reserve-list-cate').length > 0){
                $('.wrap-reserve-form .wrap-reserve-list-cate').on('click', '.reserve-link-control', function (event) {
					event.preventDefault();
					$(this).siblings('ul').slideToggle();
                });
                $('.wrap-reserve-form .wrap-reserve-list-cate .reserve-list-cate').on('click', 'li', function (event) {
                    var _this 	 = $(this),
						//_value 	 = _this.attr('value'),
						_value 	 = _this.text(),
						_content = _this.text(),
                    ...
                    _content = _content.slice(0, 12);
                    _this.parent().siblings('a').text(_content).attr('title',_title);
                    _this.parent().siblings('input[name="center_service"]').val(_value);
					_this.parent().siblings('input[name="center_service_id"]').val(_this.data("id"));
                    _this.parent().slideUp();
                });
			}
			if($("select:not(.except-chosen)").length > 0){
                ...
                $(document).on('click','.tab-control .tab-control-item', function(ev){
                    ...
                    if(!$(this).hasClass('active')){
                        var _this = $(this),
                            ...
                            _tab_active = _this.closest('.wrap-service-tab').find('.tab-contents').find(_link_content);
                        _this.siblings(".active").removeClass('active');
                        _this.addClass('active');
                        _this.closest('.wrap-service-tab').find('.tab-contents .active').removeClass('active');
                        _tab_active.addClass('active');
                        _tab_active.find('.wrap-services .owl-item').each(function (index) {
                            var owl_item = $(this),
		...
		mercado_price_quantity: function(){
			if($(".quantity-input").length > 0){
				$(".quantity-input").on('click', '.btn', function(event) {
					event.preventDefault();
					var _this = $(this),
						_input = _this.siblings('input[name=service-quatity]'),
						_current_value = _this.siblings('input[name=service-quatity]').val(),
						_max_value = _this.siblings('input[name=service-quatity]').attr('data-max');
		...
		mercado_remove_service_in_cart: function(){
			if($(".services-cart .pr-cart-item").length > 0){
